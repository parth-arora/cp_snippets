...
vector<int> brute(const vector<int>& left, const vector<int>& right) {
    int ssss = left.size() + right.size() - 1;
    vector<int> ret(ssss, 0);

    for (int i = 0; i < left.size(); i++) {
        for (int j = 0; j < right.size(); j++) {
            ret[i+j] = (ret[i+j] + left[i]*1ll*right[j]) % mod;
        }
    }
...
vector<int> rec(int st, int ed) {
    if (st == ed) {
        return vector<int>{st, 1};
    }

    int md = (st+ed)/2;
    vector<int> left = rec(st, md);
    vector<int> right = rec(md+1, ed);
